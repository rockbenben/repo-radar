...
**`.git` 不是目录时必须返回「不可缓存」**：worktree 与 submodule 的 `.git` 是一个文件，六个 probe 全部 stat 失败。此时若返回一个恒定字符串，这类仓库会**永远命中缓存、heavy 永不刷新**，且没有任何报错——一个只在少数用户身上出现的静默失效。因此 `gitFingerprint` 的返回类型是 `string | null`，`null` 表示调用方必须当作永远未命中。
...
- 键：`repoId`；值：`{ fingerprint, heavy, seenAt }`。
- 命中条件：`fingerprint` 完全相等。任何不等、为 `null`、或读取失败一律视为未命中，走完整路径。
- 每轮全量重扫后剪枝，但**必须带 30 天年龄护栏**，沿用 `desc-cache.ts:73` 的既有理由：网络盘根目录瞬时掉线会让一整批仓库在某轮扫描里消失，立即剪会把它们的落盘数据永久抹掉。这对身份账本尤其致命——账本条目被剪掉后，那批仓库回来时会被当成全新仓库，正是本轮要消灭的行为。故 `repo-cache.json` 与 `repo-identity.json` 的条目都带 `seenAt`。
...
- **同一路径上 `ino` 变了**（重新 clone、从备份恢复）：路径在账本里命中即视为同一仓库，身份不变，根本不进认领流程。取这一侧是因为「删掉重新 clone 同一个仓库到原路径」很常见，若因 `ino` 变了就作废身份，用户的标签/收藏/归档会莫名消失；而「删掉后在同一路径新建一个无关仓库」罕见得多。

  **唯一的例外**（实施评审发现，用户拍板补上）：同一轮扫描里既发生「A 改名成 B」又在腾空的路径 A 上新建了无关仓库时，路径命中会让**新仓库继承 A 的标签/归档**，而真正改名的 B 铸造新 id、什么都不剩——静默的错误数据。判别信号是「已知路径存的 `(dev,ino)` 与当前实际 stat 对不上，**且**本轮某个未知路径正好 stat 出那个老 `(dev,ino)`」。这条不回归上面的 re-clone 行为：重新 clone 时 `ino` 同样会变，但没有任何未知路径带着老 `ino`，路径命中照样赢。

- **同轮次约束**（用户拍板）：只允许认领「上一代还被盖过章、这一代路径没了」的 id，`gen` 持久化在账本条目里。动机是判据②按根提交匹配却**不能比较 `dev`**（跨卷移动时 `dev` 本来就变了，比了判据②就失去意义），于是两个 clone 分处 lost/found 两侧时会互相认领；不加约束这个窗口是 `prune` 的整整 30 天，加了之后压到一轮。代必须持久化——用内存计数器会让「关掉应用 → 改名 → 重新打开」这个主用例坏掉，而那正是改名最常发生的时机。
