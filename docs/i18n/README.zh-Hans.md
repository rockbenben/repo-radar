...
- **深入某个仓库** —— 详情面板给出完整的健康明细、切换 / 新建 / 丢弃分支、带实时 diff 的 **就地提交**、按需拉取的 GitHub PR 与 CI、最近提交、stash、12 周热力图，以及对已合并分支的一键清理——它只在你处于 `main`/`master` 时提供，因为只有站在主干上，「已合并」才等于「已并进主干」。丢弃改动会还原已跟踪文件、删除未跟踪文件，但不动 submodule 内容与未跟踪的嵌套 git 仓库；有东西没能丢掉时它会如实说明，而不是报成功。
- **保持最新** —— 默认的刷新方式是 30 分钟一轮的兜底重扫，加上顶栏的手动重扫。文件监听自动扫描**默认关闭**，可在设置面板自行开启：它纯本地、不走网络，但几个项目同时在跑构建时内核通知缓冲区会持续溢出，每次溢出都要补一轮重扫——对一个「看一眼哪里变了」的工具来说，这笔常驻开销不值得。开启之后，Windows 和 macOS 上，每个扫描目录只用一个递归监听句柄就能覆盖它下面的全部仓库，新增、删除或改名一个仓库几秒内就能被发现；Linux 上仓库是逐个监听的，`watchLimit`（默认 200，0 = 无上限）限制监听数量，优先给收藏和最近提交的仓库。监听持续溢出时，补票重扫会指数退避（最长 30 分钟一轮），也不再拆建整套监听句柄——只有监听目标真的没了才重建。兜底重扫补上监听漏掉的事件，顶栏显示「上次扫描」是多久以前，设置面板实时显示「N 个中监听 M 个」。改名或搬移一个仓库，它的标签、收藏、归档状态和便签都会保留——repo-radar 认的是仓库身份，不是路径。身份是在搬移之后**紧接着那一轮**扫描里认领的，于是有两个缺口：一是跨盘的慢速搬移横跨两轮扫描、中间恰好撞上别的仓库增删或一次周期重扫；二是新位置在那一轮根本没被扫到——先把仓库移出扫描目录、之后才把新位置加成扫描目录，是最常见的撞法。两者都会退回按路径识别：仓库以一张全新卡片回来，而标签/收藏/归档/便签留在它已经不再持有的那个 id 下。定时后台 fetch 需自行开启。一个 **统计** tab（一年的提交热力图、最活跃/最不活跃）和一个 **工作记录** tab，可把某个日期范围复制成 Markdown 周报。
...
关闭窗口只会把 repo-radar 收进托盘，于是兜底重扫、文件监听（若你开了）、定时 fetch 和 GitHub 提醒都继续运行——点托盘图标把看板唤回，或从托盘菜单里真正退出。（在 Linux 上，桌面托盘并不可靠，所以关闭就是退出；用 开机自启 让它常驻。）

退出时会等已经在跑的 git 操作收尾——批量 pull、丢弃 stash、定时 fetch——上限 10 秒，以免写入被切断、留下一个过期的 `.git/index.lock`。等不到也会照样退出，并在日志里说明：你之后撞上 `index.lock` 时，那行日志是唯一能解释原因的东西。
...
| 字段 | 作用 |
| --- | --- |
| `roots` / `excludes` / `manualRepos` | 扫描哪里（向下 6 层内查找 `.git`，不跟随软链）、跳过什么，以及在 roots 之外添加的仓库——`manualRepos` 里的仓库改名或搬移不会像扫描到的仓库那样被身份账本认回，卡片会报错，需要你来这里手动更新路径；若搬移已经过了一轮扫描，更新路径能让卡片回来，但标签/收藏/归档/便签不会跟着回来 |
| `health` | `{ staleDays, disabledRules }` —— 调整「长期未动」阈值或禁用单项检查 |
| `open` | 编辑器 / 终端 / 目录 按钮的命令模板（`{path}` = 仓库路径） |
| `autoWatch` / `autoScanMinutes` / `watchLimit` / `autoFetchMinutes` / `notifications` | 后台行为——只有 `autoScanMinutes`（默认 30 分钟）默认开启，另外三个（含 `autoWatch`）默认关闭。`watchLimit`（默认 200，0 = 无上限）只在 Linux 上有意义——那里仓库是逐个监听的；Windows 和 macOS 每个扫描目录用一个递归监听句柄，恒为全覆盖 |
| `tags` / `favorites` / `groupOverrides` / `notes` / `archived` | 按仓库的组织信息 |

`config.json` 同目录下还有两个文件，删掉都安全——repo-radar 会重新建起来，只是代价不同。`repo-cache.json` 按 `.git` 指纹记住每个仓库的「重」字段（stash、tag、remote、最近提交、已合并分支……），仓库没动过时下一轮重扫就跳过这些 git 调用；删掉它，下一轮重扫只是慢一次。`repo-identity.json` 是身份账本，让改名或搬移后的仓库继续认得自己的标签、收藏、归档状态和便签，而不是被当成全新仓库。删掉它，损失是立刻发生的，不是延后的：任何在账本丢失之前就已经改过名或搬过家的仓库，下一次扫描就会被当成全新仓库、铸造一个全新 id，原来的标签/收藏/归档/便签全部还挂在那个不再属于它的旧 id 上，找不回来了。从没改过名的仓库不受影响；账本重新积累之后，之后发生的改名同样会被正常保护。
