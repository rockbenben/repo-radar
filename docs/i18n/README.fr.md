...
- **Rester à jour** — le scan automatique par surveillance de fichiers est activé par défaut (local uniquement, sans réseau ; `node_modules` et les dossiers de sortie de build comme `dist` / `obj` / `target` sont ignorés), doublé d'un rescan toutes les 30 minutes pour les événements manqués et d'un indicateur « dernier scan » dans la barre ; au-delà de la limite de surveillance (200 dépôts par défaut, réglable jusqu'à illimité), les favoris et les dépôts récemment commités sont surveillés en priorité, les autres passent par le rescan — le panneau de réglages affiche la couverture réelle (« M sur N surveillés »). Le fetch programmé en arrière-plan est optionnel. Un onglet **Stats** (heatmap des commits sur un an, les plus/moins actifs) et un onglet **Worklog** qui copie une plage de dates sous forme de rapport hebdomadaire Markdown.
...
| `autoWatch` / `autoScanMinutes` / `watchLimit` / `autoFetchMinutes` / `notifications` | comportement en arrière-plan — `autoWatch` activé, `autoScanMinutes` à 30 et `watchLimit` à 200 par défaut (0 = illimité), les deux autres désactivés |
