...
- **최신 유지** — 파일 감시 자동 스캔은 기본으로 켜져 있고(로컬 전용, 네트워크 사용 없음. `node_modules`와 `dist` / `obj` / `target` 같은 빌드 산출물 디렉터리는 제외), 감시가 놓친 변경은 30분마다 도는 재스캔이 받아 주며 툴바에 「마지막 스캔」이 표시됩니다. 감시 상한(기본 200, 조절 가능하며 무제한도 가능)을 넘으면 즐겨찾기와 최근 커밋된 저장소를 우선 감시하고 나머지는 재스캔이 담당하며, 설정 패널에 「N개 중 M개 감시」가 그대로 표시됩니다. 예약된 백그라운드 fetch는 직접 켜야 합니다. 그리고 **Stats** 탭(1년치 커밋 히트맵, 가장 활발한/가장 덜 활발한)과 날짜 범위를 Markdown 주간 보고서로 복사하는 **Worklog** 탭이 있습니다.
...
| `autoWatch` / `autoScanMinutes` / `watchLimit` / `autoFetchMinutes` / `notifications` | 백그라운드 동작 — `autoWatch` 켜짐, `autoScanMinutes` 30분, `watchLimit` 200이 기본값(0 = 무제한), 나머지 둘은 꺼짐 |
