...
| Field | What it does |
| --- | --- |
| `roots` / `excludes` / `manualRepos` | where to scan (finds `.git` up to 6 deep), what to skip, and repos added outside the roots — a `manualRepos` entry that gets renamed or moved isn't tracked by identity like a scanned repo is; the card errors until you update its path here |
